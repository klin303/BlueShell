...
let translate (stmts, functions) =
  ...
  let rec expr (curr_symbol_table : symbol_table) function_decls builder ((_, e) : sexpr) =
    ...
    | SId s ->
        ...
        (curr_symbol_table, function_decls, L.build_load address s builder)
    | SChar c ->
      ...
      (curr_symbol_table, function_decls, dbl_char_ptr)
    | SString s ->
      ...
      (curr_symbol_table, function_decls, dbl_string_ptr)
    | SNoexpr -> (curr_symbol_table, function_decls, L.const_int i32_t 0)
    | SExec (e1, e2) -> let struct_space = L.build_malloc exec_t "struct_space" builder in
                        ...
                        let (_, _, new_value) = (expr curr_symbol_table function_decls builder e1) in
    ...
    | SBinop (e1, op, e2) ->
      ...
      in let (curr_symbol_table'', new_function_decls', e2') = expr curr_symbol_table' new_function_decls builder e2 in
      (match op with
        ExprAssign ->
          let (new_symbol_table, new_function_decls'', ptr) = expr curr_symbol_table new_function_decls' builder e1
          in let (_, _, e2') = expr curr_symbol_table new_function_decls'' builder e2 in
          ...
          in
          (new_symbol_table, new_function_decls''', e2')
        | Add -> (match t with
          Float ->
    ...
    | SList l -> (match l with
      [] -> (curr_symbol_table, function_decls, L.const_pointer_null (L.pointer_type list_t))
      ...
      | first :: rest ->
                        ...
                        | _  -> L.const_int i32_t 5
                        in
                        let (_, function_decls', value) = expr curr_symbol_table function_decls builder first
                        in
                        ...
                        let (_, function_decls'',list_ptr) = expr curr_symbol_table function_decls' builder (List_type (fst first), SList(rest))
                        in
      ...
      | SAssign (s, e) -> let address = lookup curr_symbol_table s in
                          let (_, function_decls', e') = expr curr_symbol_table function_decls builder e in
                          ...
                          let new_function_decls = (match (fst e) with
                            Function _ -> (match (snd e) with
                                            SId s1 -> let mapping = StringMap.find s1 function_decls'
                                               ...
                                               | _  -> raise (Failure "Only ids can be assigned"))
                           | _ -> function_decls')
                          in (curr_symbol_table, new_function_decls, e')
                          (* (match address with
      ...
      | SBind (ty, n)  ->
            ...
            ({ variables = new_sym_table ; parent = curr_symbol_table.parent }, function_decls, ptr)
        (* (match ty with
      ...
      | SCall (f, args) ->
        ...
        let llargs = List.map third (List.rev (List.map (expr curr_symbol_table function_decls builder) (List.rev args))) in
        let result = (match fdecl.styp with
                      A.Void -> ""
                    | _ -> f ^ "_result") in
        (curr_symbol_table, function_decls, L.build_call fval (Array.of_list llargs) result builder)
  ...
  let rec stmt ((curr_symbol_table : symbol_table), (function_decls : (L.llvalue * sfunc_decl) StringMap.t), builder, (fdecl_option: sfunc_decl option), (func_llvalue : L.llvalue)) (statement : sstmt) =
      ...
      SReturn e -> (match fdecl_option with
        Some(fdecl) ->  (match fdecl.styp with
                      ...
                      | _ -> let ret_mem = L.build_malloc (ltype_of_typ fdecl.styp) "return malloc" builder in
                              let ret = L.build_load (third (expr curr_symbol_table function_decls builder e)) "return load" builder in
    ...
    | SExpr e -> let (new_symbol_table, new_function_decls, expr_val) = expr curr_symbol_table function_decls builder e in (new_symbol_table, new_function_decls, builder, fdecl_option, func_llvalue)
    | SIf (predicate, then_stmt, else_stmt) ->
      let (curr_symbol_table', new_function_decls, bool_val) = expr curr_symbol_table function_decls builder predicate in
